...
#if UNITY_EDITOR
[CustomEditor(typeof(EasyMultiplyParamEvent))]
[CanEditMultipleObjects]
class EasyMultiplyParamEventEditor : Editor
{
    ...
    {
        var t = (target as EasyMultiplyParamEvent);
        ...
        if (t.EventObject)
        {
            ...
            if (ParametersLength.intValue > 0 && t.EventComponent != null && EventMethod.stringValue != null)
            {
                for (int i = 0; i < ParametersLength.intValue; i++)
                {
                    ...
                    System.Type _type = t.EventObject.GetComponent(EventComponent.stringValue).GetType().GetMethod(EventMethod.stringValue).GetParameters()[i].ParameterType;
